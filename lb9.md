...
Паролі не зберігаються у відкритому вигляді з міркувань безпеки. Вони зберігаються у зашифрованому вигляді у файлі /etc/shadow. Це потрібно для того, щоб у разі доступу до файлу неможливо було дізнатися справжній пароль користувача.

---

**Чому не рекомендується виконувати повсякденні операції, використовуючи обліковий запис root?**  

Тому що root має повні права доступу до системи. Будь-яка помилка може призвести до пошкодження системи або видалення важливих файлів, тому для звичайної роботи краще використовувати звичайного користувача.

---

**У чому відмінність механізмів su і sudo?**  

- Команда **su** використовується для повного переходу на іншого користувача (зазвичай root).  
- Команда **sudo** дозволяє виконати одну команду з правами адміністратора без переходу в root.
  
Тобто sudo є більш безпечним способом.

---

**Чому домашній каталог користувача root не розміщено в каталозі /home?**  

Домашній каталог root знаходиться у /root для ізоляції від звичайних користувачів. Це підвищує безпеку та дозволяє root працювати навіть у випадку проблем із каталогом /home.

---

**Для чого використовується команда getent?**  

Команда **getent** використовується для отримання інформації про користувачів і групи з системних баз даних.
...
Пароль користувача можна змінити за допомогою команди:
```bash
passwd
```
...
Команда chage використовується для керування строком дії пароля користувача (дата зміни, термін дії, блокування).
```bash
chage -l $(whoami)
```
...
Найчастіше використовуються такі параметри:
- -aG - додавання користувача до групи
- -l - зміна імені користувача
- -d - зміна домашньої директорії
